...
/** \brief Class PsTypeChunk generated.
 ...
 * to differentiate parameters when two \ref PsTypeChunk are
 ...
 * \code
 myObject
 {
   Class ObjectWhichUsesPsTypeChunk
   Scheduling
   {
     Frequency 75
     Process processA
 ...
 * In the header:
 * \code
 class ObjectWhichUsesPsTypeChunk : public SimulatedObject
 {
   //...
  protected:
   PsTypeChunk _psTypeChunk1 ;
   PsTypeChunk _psTypeChunk2 ;
 } ;
 * \endcode
 * In the source:
 * \code
 void ObjectWhichUsesPsTypeChunk::init()
 {
   //...
   _psTypeChunk1.loadParameters( this, "first" ) ;
   _psTypeChunk2.loadParameters( this, "second" ) ;
 }
 * \endcode
 ...
 * It is easier to create configuration with multiple instances of PsTypeChunk
 ...
 * \code
 void ObjectWhichUsesPsTypeChunk::init()
 {
   ...
   _psTypeChunk1.loadParameters( node ? node->getSubDescriptorByName( "firstPsTypeChunk"  ) : 0, "", this ) ;
...
public:
	...
	//@{
	/// \brief Default constructor of\ref PsTypeChunk.
	PsTypeChunk();
...
public:
	...
	/// \param[in] p the simulated object use by the \ref PsParameterAccessor to display error messages.
	...
	bool loadParameters( const PsConfigurationParameterDescriptor * node, 
			const std::string& prefix = "",
			PsSimulatedObject* p = 0 ) ;

	/// \brief Configuration parameters loader of \ref PsTypeChunk.
	/// \overload
	/// \n The used node is the user param node of the simulated object
	bool loadParameters( PsSimulatedObject* p, const std::string& prefix = "" ) ;

protected:
	/// \brief Protected loader, does not reset the default values
	virtual bool innerParametersLoader( const PsConfigurationParameterDescriptor * node,
					const std::string& prefix,
					PsSimulatedObject* p ) ;
	//@}
...
public:
	...
	/// \brief Return the \b buffer value of \ref PsTypeChunk.
	const OpenViBE::Kernel::Player::CBuffer& getBuffer() const { return _buffer ; }
	OpenViBE::Kernel::Player::CBuffer& getBuffer() { return _buffer ; }
	//@}

	/// @name Operators.
	//@{
	/// \brief Comparison operator for equality
	bool operator == ( const PsTypeChunk& ref ) const { return !( *this != ref ) ; } 
	/// \brief Comparison operator for difference
	bool operator != ( const PsTypeChunk& ref ) const ;
	/// \brief Copy operator 
	PsTypeChunk& operator = ( const PsTypeChunk& ref ) ;
	//@}

	/// @name PsPackable, PsFlowable, PsType interfaces.
	//@{
public:
...
protected:
	...
	void _copy( const PsTypeChunk& ref ) ;
