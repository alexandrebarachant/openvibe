	...
	const CIdentifier& rBoxIdentifier)
{
	...
	for(itLink=m_vLink.begin(); itLink!=m_vLink.end(); )
	{
		...
		if(itLinkCurrent->second->getSourceBoxIdentifier()==rBoxIdentifier || itLinkCurrent->second->getTargetBoxIdentifier()==rBoxIdentifier)
		{
			...
			// Deletes this link
			OpenViBE::Tools::CObjectFactoryHelper(getKernelContext().getObjectFactory()).releaseObject(itLinkCurrent->second);
	...
	CIdentifier& rProcessingUnitIdentifier)
{
	...
	IProcessingUnit* l_pProcessingUnit=OpenViBE::Tools::CObjectFactoryHelper(getKernelContext().getObjectFactory()).createObject<IProcessingUnit*>(OV_ClassId_Kernel_ProcessingUnit);
	...
	const CIdentifier& rProcessingUnitIdentifier)
{
	...
	OpenViBE::Tools::CObjectFactoryHelper(getKernelContext().getObjectFactory()).releaseObject(itProcessingUnit->second);
