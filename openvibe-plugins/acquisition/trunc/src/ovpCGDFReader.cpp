...
namespace OpenViBEPlugins
{
	...
	{	
		...
		OpenViBE::boolean CGDFReader::initialize()
		{
			...
			OpenViBE::CString l_sParam;
			l_pBoxContext->getSettingValue(1, l_sParam); 
			m_ui32SamplesPerBuffer = static_cast<OpenViBE::uint32>(atoi((const char*)l_sParam));

			
			//Prepares the writers proxies
			m_pOutputWriterCallbackProxy[GDFReader_ExperimentInfoOutput] = new EBML::TWriterCallbackProxy1<OpenViBEPlugins::Acquisition::CGDFReader>(*this, &CGDFReader::writeExperimentOutput);
		
			m_pOutputWriterCallbackProxy[GDFReader_SignalOutput] = new EBML::TWriterCallbackProxy1<OpenViBEPlugins::Acquisition::CGDFReader>(*this, &CGDFReader::writeSignalOutput);
			
			m_pOutputWriterCallbackProxy[GDFReader_StimulationOutput] = new EBML::TWriterCallbackProxy1<OpenViBEPlugins::Acquisition::CGDFReader>(*this, &CGDFReader::writeStimulationOutput);
		...
		OpenViBE::boolean CGDFReader::uninitialize()
		{
		...
		OpenViBE::boolean CGDFReader::processClock(OpenViBE::CMessageClock& rMessageClock)
		{
		...
		void CGDFReader::GDFBufferToFloat64Buffer(OpenViBE::float64 * out, void * in, OpenViBE::uint64 inputBufferSize, OpenViBE::uint32 ui32Channel)
		{	
			switch (m_pChannelType[ui32Channel])
			{
				case GDF::ChannelType_int8 : 
					GDFTypeBufferToFloat64Buffer<OpenViBE::int8>(out, reinterpret_cast<OpenViBE::int8 *>(in), inputBufferSize, ui32Channel);
				break;
					
				case GDF::ChannelType_uint8 : 
					GDFTypeBufferToFloat64Buffer<OpenViBE::uint8>(out, reinterpret_cast<OpenViBE::uint8 *>(in), inputBufferSize, ui32Channel);
				break;
					
				case GDF::ChannelType_int16 : 
					GDFTypeBufferToFloat64Buffer<OpenViBE::int16>(out, reinterpret_cast<OpenViBE::int16 *>(in), inputBufferSize, ui32Channel);
				break;	
					
				case GDF::ChannelType_uint16 : 
					GDFTypeBufferToFloat64Buffer<OpenViBE::uint16>(out, reinterpret_cast<OpenViBE::uint16 *>(in), inputBufferSize, ui32Channel);
				break;	
					
				case GDF::ChannelType_int32 :
					GDFTypeBufferToFloat64Buffer<OpenViBE::int32>(out, reinterpret_cast<OpenViBE::int32 *>(in), inputBufferSize, ui32Channel);
				break;	
					
				case GDF::ChannelType_uint32 :
					GDFTypeBufferToFloat64Buffer<OpenViBE::uint32>(out, reinterpret_cast<OpenViBE::uint32 *>(in), inputBufferSize, ui32Channel);
				break;	
					
				case GDF::ChannelType_int64 :
					GDFTypeBufferToFloat64Buffer<OpenViBE::int64>(out, reinterpret_cast<OpenViBE::int64 *>(in), inputBufferSize, ui32Channel);
				break;	
					
				case GDF::ChannelType_uint64 :
					GDFTypeBufferToFloat64Buffer<OpenViBE::uint64>(out, reinterpret_cast<OpenViBE::uint64 *>(in), inputBufferSize, ui32Channel);
				break;	
				
				case GDF::ChannelType_float32 :
					GDFTypeBufferToFloat64Buffer<OpenViBE::float32>(out, reinterpret_cast<OpenViBE::float32 *>(in), inputBufferSize, ui32Channel);
				break;	
					
				case GDF::ChannelType_float64 :
					GDFTypeBufferToFloat64Buffer<OpenViBE::float64>(out, reinterpret_cast<OpenViBE::float64 *>(in), inputBufferSize, ui32Channel);
				break;	
				...
				{ 
					OpenViBE::uint8 * l_pIn = reinterpret_cast<OpenViBE::uint8 *>(in);
					
					for(OpenViBE::uint64 i = 0 ; i<(inputBufferSize*3) ; i+=3)
					{
				...
				{ 
					OpenViBE::int8 * l_pIn = reinterpret_cast<OpenViBE::int8 *>(in);
		...
		void CGDFReader::writeEvents()
		{
			...
			//m_pWriterHelper[GenericNetworkAcquisition_SignalOutput]->setBinaryAsChildData(static_cast<void*> (m_pMatrixBuffer), m_ui64MatrixBufferSize*sizeof(EBML::float64));
		}
		
		OpenViBE::boolean CGDFReader::process()
		{	
			...
			else if (!m_bSignalDescriptionSent)
			{
				...
				OpenViBE::float64 * l_pPhysicalMinimun = reinterpret_cast<OpenViBE::float64*>(l_pVariableHeaderBuffer+(104*m_ui16NumberOfChannels));

				OpenViBE::float64 * l_pPhysicalMaximun = reinterpret_cast<OpenViBE::float64*>(l_pVariableHeaderBuffer+(112*m_ui16NumberOfChannels));

				OpenViBE::int64 * l_pDigitalMinimun = reinterpret_cast<OpenViBE::int64*>(l_pVariableHeaderBuffer+(120*m_ui16NumberOfChannels));

				OpenViBE::int64 * l_pDigitalMaximun = reinterpret_cast<OpenViBE::int64*>(l_pVariableHeaderBuffer+(128*m_ui16NumberOfChannels));
				...
				OpenViBE::uint32 * l_pNumberOfSamplesPerRecordArray = reinterpret_cast<OpenViBE::uint32*>(l_pVariableHeaderBuffer+(216*m_ui16NumberOfChannels));
				...
				m_pChannelType = new OpenViBE::uint32[m_ui16NumberOfChannels];
				memcpy(m_pChannelType, l_pVariableHeaderBuffer+ (220*m_ui16NumberOfChannels), m_ui16NumberOfChannels * 4);
				

				m_pChannelDataSize = new OpenViBE::uint16[m_ui16NumberOfChannels];
			...
			else if(!m_bMatricesSent)
			{
				...
				OpenViBE::uint32 l_ui32CurrentSampleInOutputMatrix  = 0;
				OpenViBE::boolean l_bMatrixReadyToSend = false;
				
				while(!l_bMatrixReadyToSend)
				{
					//there is the same number of samples
					OpenViBE::uint64 l_ui64SamplesRemainingInDataRecord = m_ui32NumberOfSamplesPerRecord - m_ui32CurrentSampleInDataRecord;
						...
						else
						{
							...
							for(int i=0 ; i<m_ui16NumberOfChannels ; i++)
							{
								memset(m_pMatrixBuffer + (((i*m_ui32SamplesPerBuffer) +l_ui32CurrentSampleInOutputMatrix)),
										0,
										(m_ui32SamplesPerBuffer - l_ui32CurrentSampleInOutputMatrix) * sizeof(OpenViBE::float64));
							}
			...
			else if(!m_bEventsSent)
			{
				...
				if(!m_pEventsPositionBuffer)
				{
					...
					OpenViBE::uint8 l_pEventTableHeader[7];
					...
					{
						m_ui32NumberOfEvents = *(reinterpret_cast<OpenViBE::uint32*>(l_pEventTableHeader +3));
					}
					
					m_pEventsPositionBuffer = new OpenViBE::uint32[m_ui32NumberOfEvents * 4];
					m_pEventsTypeBuffer = new OpenViBE::uint16[m_ui32NumberOfEvents * 2];
