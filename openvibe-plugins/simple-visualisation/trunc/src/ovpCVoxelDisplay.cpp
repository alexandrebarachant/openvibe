...
const uint32 g_nbVoxels = 2394;
float32 g_voxelPositions[3*g_nbVoxels] =
{
-52, -41, 11,
-45, -41, 11,
-38, -41, 11,
...
using namespace OpenViBEPlugins;
using namespace SimpleVisualisation;
...
boolean CVoxelDisplay::process3D()
{
	...
	else //proceed with color/shape modification
	{
		...
		if(m_bToggleColorModification == true)
		{
			...
			{
				//reset color
				for(uint32 i=0; i<m_oVoxels.size(); i+=l_ui32VoxelStep)
				{
...
boolean CVoxelDisplay::createVoxels()
{
	...
	//create voxels
	m_oVoxels.resize(g_nbVoxels);
	CNameValuePairList l_oParamsList;
	l_oParamsList.setValue("CloneMaterials", true);

	for(uint32 i=0; i<g_nbVoxels; i+=l_ui32VoxelStep)
	{
		//object is visible by default
		m_oVoxels[i].m_bVisible = true;
		//store voxel position
		m_oVoxels[i].setPosition(
			(g_voxelPositions[3*i] - l_f32VoxelOffsetX) * l_f32ScaleFromOffset + l_f32VoxelOffsetX,
			(g_voxelPositions[3*i+1] - l_f32VoxelOffsetY) * l_f32ScaleFromOffset + l_f32VoxelOffsetY,
			(g_voxelPositions[3*i+2] - l_f32VoxelOffsetZ) * l_f32ScaleFromOffset + l_f32VoxelOffsetZ);
