...
namespace OpenViBEPlugins
{
	namespace Utility
	{
		...
		OpenViBE::boolean GDF::CFixedGDF1Header::save(std::ofstream& oFile)
		{
			if(oFile.is_open())
			{
				OpenViBE::uint8 l_pTempBuffer[sizeof(OpenViBE::uint64)];
				...
				System::Memory::hostToLittleEndian(static_cast<OpenViBE::uint64>(m_i64NumberOfBytesInHeaderRecord), l_pTempBuffer);
				oFile.write(reinterpret_cast<char *>(l_pTempBuffer), sizeof(OpenViBE::int64));
				
				System::Memory::hostToLittleEndian(static_cast<OpenViBE::uint64>(m_ui64EquipmentProviderId), l_pTempBuffer);
				oFile.write(reinterpret_cast<char *>(l_pTempBuffer), sizeof(OpenViBE::uint64));
					
				System::Memory::hostToLittleEndian(static_cast<OpenViBE::uint64>(m_ui64LaboratoryId), l_pTempBuffer);
				oFile.write(reinterpret_cast<char *>(l_pTempBuffer), sizeof(OpenViBE::uint64));
				
				System::Memory::hostToLittleEndian(static_cast<OpenViBE::uint64>(m_ui64TechnicianId), l_pTempBuffer);
				oFile.write(reinterpret_cast<char *>(l_pTempBuffer), sizeof(OpenViBE::uint64));
				
				oFile.write(m_sReservedSerialNumber, sizeof(m_sReservedSerialNumber));
				
				System::Memory::hostToLittleEndian(static_cast<OpenViBE::uint64>(m_i64NumberOfDataRecords), l_pTempBuffer);
				oFile.write(reinterpret_cast<char *>(l_pTempBuffer), sizeof(OpenViBE::int64));
				
				System::Memory::hostToLittleEndian(m_ui32DurationOfADataRecordNumerator, l_pTempBuffer);
				oFile.write(reinterpret_cast<char *>(l_pTempBuffer), sizeof(OpenViBE::uint32));
		...
		OpenViBE::boolean GDF::CFixedGDF1Header::update(std::ofstream& oFile)
		{
			if(oFile.is_open())
			{
				OpenViBE::uint8 l_pTempBuffer[sizeof(OpenViBE::uint64)];
				
				OpenViBE::uint64 l_oBackupPosition = oFile.tellp();
		...
		OpenViBE::boolean GDF::CFixedGDF2Header::save(std::ofstream& oFile)
		{
			return false;
			//TODO complete
			/*
		...
		OpenViBE::boolean GDF::CVariableGDF1Header::update(std::ofstream& oFile)
		{
			if(oFile.is_open())
			{
				OpenViBE::uint8 l_pTempBuffer[sizeof(OpenViBE::uint64)];
				...
				OpenViBE::uint32 l_ui32ChannelCount = m_vVariableHeader.size();
				...
				for(i=m_vVariableHeader.begin() ; i!=m_vVariableHeader.end() ; i++)
				{
					...
					oFile.write(reinterpret_cast<char *>(l_pTempBuffer), sizeof(OpenViBE::float64));
				}
				for(i=m_vVariableHeader.begin() ; i!=m_vVariableHeader.end() ; i++)
				{
				...
				for(i=m_vVariableHeader.begin() ; i!=m_vVariableHeader.end() ; i++)
				{
					System::Memory::hostToLittleEndian(static_cast<OpenViBE::uint64>(i->m_i64DigitalMinimum), l_pTempBuffer);
					oFile.write(reinterpret_cast<char *>(l_pTempBuffer), sizeof(OpenViBE::int64));
				}
				...
				for(i=m_vVariableHeader.begin() ; i!=m_vVariableHeader.end() ; i++)
				{
					System::Memory::hostToLittleEndian(static_cast<OpenViBE::uint64>(i->m_i64DigitalMaximum), l_pTempBuffer);
					oFile.write(reinterpret_cast<char *>(l_pTempBuffer), sizeof(OpenViBE::int64));
				}
		...
		void GDF::CVariableGDF1Header::setChannelCount(OpenViBE::uint32 ui32ChannelCount)
		{
			m_vVariableHeader.resize(ui32ChannelCount);
		}
		
		GDF::CVariableGDF1Header::CVariableGDF1HeaderPerChannel& GDF::CVariableGDF1Header::operator[](OpenViBE::uint32 ui32Channel)
		{
			return m_vVariableHeader[ui32Channel];
		}
		
		OpenViBE::boolean GDF::CVariableGDF1Header::save(std::ofstream& oFile)
		{
			if(oFile.is_open())
			{
				OpenViBE::uint8 l_pTempBuffer[sizeof(OpenViBE::uint64)];
