#include "ovtkCAlgorithmClassifierProcessor.h"
...
boolean CAlgorithmClassifierProcessor::process(void)
{
	TParameterHandler < IMatrix* > ip_pFeatureVector(this->getInputParameter(OVTK_Algorithm_ClassifierProcessor_InputParameterId_FeatureVector));
	TParameterHandler < IMemoryBuffer* > ip_pConfiguration(this->getInputParameter(OVTK_Algorithm_ClassifierProcessor_InputParameterId_Configuration));
	TParameterHandler < float64 > op_pClass(this->getOutputParameter(OVTK_Algorithm_ClassifierProcessor_OutputParameterId_Class));
	TParameterHandler < IMatrix* > op_pClassificationValues(this->getOutputParameter(OVTK_Algorithm_ClassifierProcessor_OutputParameterId_ClassificationValues));

	if(this->isInputTriggerActive(OVTK_Algorithm_ClassifierProcessor_InputTriggerId_Classify))
	{
		...
		if(!l_pFeatureVector || !l_pClassificationValues)
		{
			...
			this->activateOutputTrigger(OVTK_Algorithm_ClassifierProcessor_OutputTriggerId_Failed, true);
		}
		else
		{
			...
			if(this->classify(l_oFeatureVectorAdapter, l_f64Class, l_oClassificationValuesAdapter))
			{
				...
				this->activateOutputTrigger(OVTK_Algorithm_ClassifierProcessor_OutputTriggerId_Success, true);
			}
			else
			{
				this->activateOutputTrigger(OVTK_Algorithm_ClassifierProcessor_OutputTriggerId_Failed, true);
	...
	if(this->isInputTriggerActive(OVTK_Algorithm_ClassifierProcessor_InputTriggerId_LoadConfiguration))
	{
