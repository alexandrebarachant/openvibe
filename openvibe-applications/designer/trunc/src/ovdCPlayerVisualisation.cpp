...
CPlayerVisualisation::~CPlayerVisualisation(void)
{
	...
	{
		g_signal_handlers_disconnect_by_func(G_OBJECT(it->second), reinterpret_cast<void*>(G_CALLBACK(CPlayerVisualisation::configure_event_cb)), this);
#ifdef SAVE_PANED_POSITIONS
		g_signal_handlers_disconnect_by_func(G_OBJECT(it->second), reinterpret_cast<void*>(G_CALLBACK(CPlayerVisualisation::window_state_event_cb)), this);
#endif
...
#ifdef OV_3D
	m_rKernelContext.getVisualisationManager().init3DContext();
#endif
...
{
#ifdef OV_3D
	m_rKernelContext.getVisualisationManager().release3DContext();
#endif
}
...
#ifdef SAVE_PANED_POSITIONS
			gtk_signal_connect(GTK_OBJECT(l_pWidget), "window_state_event", G_CALLBACK(window_state_event_cb), this);
#endif
...
void CPlayerVisualisation::endLoadTreeWidget(::GtkWidget* treeWidget)
{
...
boolean CPlayerVisualisation::setWidgets(const CString& rVisualisationBoxName, ::GtkWidget* pWidget, ::GtkWidget* pToolbarWidget)
{
	...
	if(m_rVisualisationTree.findChildNodeFromRoot(&l_oVisualisationBoxIter, (const char*)rVisualisationBoxName, EVisualisationTreeNode_VisualisationBox) == true)
	{
		...
		if(l_pVisualisationWidget != NULL)
		{
			...
			::GtkButton* l_pButton = GTK_BUTTON(gtk_button_new());
			//gtk_button_set_label(l_pButton, (const char*)rVisualisationBoxName);

			{
				...
				{
					m_rVisualisationTree.getStringValueFromTreeIter(&l_oIter, &l_oIconString, EVisualisationTreeColumn_StringStockIcon);
				}

				//create icon
				::GtkWidget* l_pIcon = gtk_image_new_from_stock(l_oIconString != NULL ? l_oIconString : GTK_STOCK_EXECUTE, GTK_ICON_SIZE_BUTTON);
			...
			g_signal_connect(G_OBJECT(l_pButton), "clicked", G_CALLBACK(button_clicked_cb), this);
			...
			//associate button to its identifier and to toolbar
			m_mVisualisationWidgetToolbar[GTK_WIDGET(l_pButton)] = pToolbarWidget;
			m_mVisualisationWidget[GTK_WIDGET(l_pButton)] = l_pVisualisationWidget->getIdentifier();
			...
			else //retrieve parent widget in which to insert VBox
			{
				...
				if(m_rVisualisationTree.findChildNodeFromRoot(&l_oParentIter, l_pVisualisationWidget->getParentIdentifier()) == true)
				{
					...
					m_rVisualisationTree.getPointerValueFromTreeIter(&l_oParentIter, (void**)&l_pParentWidget, EVisualisationTreeColumn_PointerWidget);
					CIdentifier l_oParentIdentifier;
...
void CPlayerVisualisation::hideTopLevelWindows(void)
{
	...
	if(m_pCurrentToolbar != NULL)
	{
		gtk_widget_hide_all(m_pCurrentToolbar);
	}
}

void CPlayerVisualisation::realize3DWidgets(void)
{
#ifdef OV_3D
	m_rKernelContext.getVisualisationManager().realize3DVisualisationWidgets();
#endif
}

#ifdef SAVE_PANED_POSITIONS
gboolean CPlayerVisualisation::notify_position_paned_cb(::GtkWidget *widget, GParamSpec* spec, void* user_data)
{
	static_cast<CPlayerVisualisation*>(user_data)->notifyPositionPanedCB(widget);
	return TRUE;
}

void CPlayerVisualisation::notifyPositionPanedCB(::GtkWidget* pWidget)
{
	...
	::GtkTreeIter l_oIter;
	if(m_rVisualisationTree.findChildNodeFromRoot(&l_oIter, pWidget) == true)
	{
...
void CPlayerVisualisation::enablePanedSignals(::GtkWidget* pPaned, boolean b)
{
	if(b)
	{
		g_signal_connect(G_OBJECT(pPaned), "notify::position", G_CALLBACK(CPlayerVisualisation::notify_position_paned_cb), this);
	}
	else
	{
		g_signal_handlers_disconnect_by_func(G_OBJECT(pPaned), reinterpret_cast<void*>(G_CALLBACK(CPlayerVisualisation::notify_position_paned_cb)), this);
	}
}

gboolean CPlayerVisualisation::window_state_event_cb(::GtkWidget* widget, GdkEventWindowState* event, gpointer user_data)
{
	//refresh widgets if window was maximized or minimized
	if(event->changed_mask & GDK_WINDOW_STATE_MAXIMIZED || event->changed_mask & GDK_WINDOW_STATE_ICONIFIED)
	{
		//widgets haven't been reallocated yet, perform resizing only when this happens
		gtk_signal_connect(GTK_OBJECT(widget), "size-allocate", G_CALLBACK(window_size_allocate_cb), user_data);
	}

	return FALSE;
}

void CPlayerVisualisation::window_size_allocate_cb(::GtkWidget* pWidget, ::GtkAllocation* pAllocation, gpointer user_data)
{
	g_signal_handlers_disconnect_by_func(G_OBJECT(pWidget), reinterpret_cast<void*>(G_CALLBACK(CPlayerVisualisation::window_size_allocate_cb)), user_data);

	if(GTK_IS_CONTAINER(pWidget))
	{
		static_cast<CPlayerVisualisation*>(user_data)->resizeCB(GTK_CONTAINER(pWidget));
	}
}
#endif
...
{
#ifndef SAVE_PANED_POSITIONS
	//if paned positions aren't to be saved, they are to be read once only
...
#ifdef SAVE_PANED_POSITIONS
		enablePanedSignals(GTK_WIDGET(l_pPaned), false);
#endif
...
#ifdef SAVE_PANED_POSITIONS
		enablePanedSignals(GTK_WIDGET(l_pPaned), true);
#endif
...
void CPlayerVisualisation::drag_data_received_in_widget_cb(::GtkWidget* pDstWidget, GdkDragContext* pDragContext,gint iX,gint iY,::GtkSelectionData* pSelectionData,guint uiInfo,guint uiTime,gpointer pData)
{
	...
	do
	{
		l_pSrcBoxWidget = gtk_widget_get_parent(l_pSrcWidget);
	}
...
void CPlayerVisualisation::button_clicked_cb(::GtkButton* pButton, gpointer user_data)
{
	static_cast<CPlayerVisualisation*>(user_data)->showToolbarCB(GTK_WIDGET(pButton));
}

void CPlayerVisualisation::showToolbarCB(::GtkWidget* pButton)
{
	if(m_mVisualisationWidgetToolbar.find(pButton) != m_mVisualisationWidgetToolbar.end())
	{
		//retrieve toolbar pointer
		::GtkWidget* pToolbar = m_mVisualisationWidgetToolbar[pButton];

		//hide previous toolbar, if any
		if(pToolbar != m_pCurrentToolbar && m_pCurrentToolbar != NULL)
		{
			gtk_widget_hide_all(pToolbar);
		}

		//set toolbar transient for plugin window
		::GtkWidget* l_pWidget = pButton;
		while(l_pWidget!=NULL && !GTK_IS_WINDOW(l_pWidget))
		{
			l_pWidget = gtk_widget_get_parent(l_pWidget);
		}
		if(l_pWidget != NULL && GTK_IS_WINDOW(pToolbar))
		{
			gtk_window_set_transient_for(GTK_WINDOW(pToolbar), GTK_WINDOW(l_pWidget));
		}

		//show new toolbar
		gtk_widget_show_all(pToolbar);
		m_pCurrentToolbar = pToolbar;
	}
}
