...
Section "GTK+"
	...
	IfFileExists "arch\gtk-2.24.8-dev.zip" no_need_to_download_gtk_dev
	NSISdl::download http://openvibe.inria.fr/dependencies/win32/gtk-2.24.8-dev.zip "arch\gtk-2.24.8-dev.zip"
	Pop $R0 ; Get the return value
		StrCmp $R0 "success" +3
			MessageBox MB_OK "Download failed: $R0"
			Quit
no_need_to_download_gtk_dev:
	ZipDLL::extractall "arch\gtk-2.24.8-dev.zip" "gtk"

	IfFileExists "arch\gtk-2.24.8-runtime.zip" no_need_to_download_gtk_runtime
	NSISdl::download http://openvibe.inria.fr/dependencies/win32/gtk-2.24.8-runtime.zip "arch\gtk-2.24.8-runtime.zip"
	Pop $R0 ; Get the return value
		StrCmp $R0 "success" +3
			MessageBox MB_OK "Download failed: $R0"
			Quit
