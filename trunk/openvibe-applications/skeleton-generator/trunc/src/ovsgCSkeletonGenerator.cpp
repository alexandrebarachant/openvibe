...
{
	static_cast<CSkeletonGenerator*>(pUserData)->buttonCheckCB();
}
void CSkeletonGenerator::buttonCheckCB()
{
	//Author and Company
	...
	{
		m_rKernelContext.getLogManager() << LogLevel_Info <<"-- Driver Name: VALID\n";
	...
	const boost::regex l_RegExpClassName("([a-z]|[A-Z]|[1-9])+",boost::regex::extended);
	...
	{
		m_rKernelContext.getLogManager() << LogLevel_Info <<"-- Class Name: VALID\n";
	...
	{
		m_rKernelContext.getLogManager() << LogLevel_Info <<"-- Channels: VALID\n";
	...
	const boost::regex l_RegExpSamplingFrequencies("(([1-9]+);)*([1-9]+)",boost::regex::extended);
	...
	m_sTargetDirectory = gtk_file_chooser_get_uri(GTK_FILE_CHOOSER(l_pFileChooser));
	string l_csTarget((const char*)m_sTargetDirectory);
...
#endif
	{
		...
		l_ssTextBuffer << "[   OK   ] Valid target directory: "<<m_sTargetDirectory<<"\n";
	}
	//-------------------------------------------------------------------------------------------------------------------------------------------//
	...
	if(l_bSuccess)
	{
		l_ssTextBuffer <<"SUCCESS !.\nPress OK to generate the files.\nIf you want to modify your choice(s), please press the \"Check\" button again.\nPress Cancel to quit without generating the files.";
...
{
	static_cast<CSkeletonGenerator*>(pUserData)->buttonOkCB();
}
void CSkeletonGenerator::buttonOkCB()
{
...
{
	static_cast<CSkeletonGenerator*>(pUserData)->buttonTooltipCB(pButton);
}
void CSkeletonGenerator::buttonTooltipCB(::GtkButton* pButton)
{
	::GtkWidget * l_pTooltipTextview = glade_xml_get_widget(m_pGladeInterface, "sg-driver-tooltips-textview");
	::GtkTextBuffer * l_pTextBuffer  = gtk_text_view_get_buffer(GTK_TEXT_VIEW(l_pTooltipTextview));
...
CSkeletonGenerator::CSkeletonGenerator(IKernelContext & rKernelContext, const char* sGladeXMLFileName)
: m_rKernelContext(rKernelContext)
{
	m_pGladeInterface = glade_xml_new(sGladeXMLFileName, NULL, NULL);
}
...
void CSkeletonGenerator::initializeDriverGenerator( void )
{
...
void CSkeletonGenerator::initializeAlgoGenerator( void )
{
	std::cout<< "NOT SUPPORTED YET !! " <<std::endl;
}

void CSkeletonGenerator::initializeBoxGenerator( void )
{
	std::cout<< "NOT SUPPORTED YET !! " <<std::endl;
}

void CSkeletonGenerator::go( void )
{

	::GtkWidget * l_pDialog = glade_xml_get_widget(m_pGladeInterface, "sg-selection-dialog");
	...
	::GtkWidget * l_pRadioDriver = glade_xml_get_widget(m_pGladeInterface, "sg-driver-selection-radio-button");
	::GtkWidget * l_pRadioAlgo = glade_xml_get_widget(m_pGladeInterface, "sg-algo-selection-radio-button");
	::GtkWidget * l_pRadioBox = glade_xml_get_widget(m_pGladeInterface, "sg-box-selection-radio-button");
