...
boolean CConfigurationBrainProductsBrainampSeries::preConfigure(void)
{
	...
	{
		::sprintf(l_sBuffer, "\\\\.\\BrainAmpUSB%i", i);
		::HANDLE l_pHandle=::CreateFile(l_sBuffer, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_WRITE_THROUGH, NULL);
		if(l_pHandle!=INVALID_HANDLE_VALUE)
		{
			::CloseHandle(l_pHandle);
...
void CConfigurationBrainProductsBrainampSeries::buttonChannelDetailsPressedCB(void)
{
	...
	::gtk_table_resize(l_pDialogChannelDetailsTable, 33, 5);
